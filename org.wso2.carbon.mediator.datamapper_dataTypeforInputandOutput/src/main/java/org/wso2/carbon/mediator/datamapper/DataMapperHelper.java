...
public class DataMapperHelper {
	 ...
	 * Messages.DataMapperHelper_errorMessage;
	 */
	...
	public static void mediateDataMapper(MessageContext context,
		...
		try {
			...
			SynapseConfiguration synapseConfiguration = context.getConfiguration();
			cacheDurable = context.getConfiguration().getRegistry().getConfigurationProperties().getProperty(CACHABLE_DURATION);
			cacheTime = (cacheDurable != null && !cacheDurable.isEmpty()) ? Long.parseLong(cacheDurable) : 10000;
			
			Map<String, Object> mappingResourceMap = synapseConfiguration.getdataMappingConfigurationCacheMap();
			...
			switch (DataType.fromString(outputType)) { 
				case CSV: 
					...
					break;
				default:
					//Currently we have csv and xml only, this needs to be properly fixed once we decide on datatypes and default value
					writer = WriterRegistry.getInstance().get(outputType).newInstance();
			}
			}
			else{
				// if the user didn't mention the outputType currently uses XML as the output type, this needs to be propely fixed
				writer = WriterRegistry.getInstance().get(DataType.XML.toString()).newInstance();
			}
					...
					if (resultQName.getLocalPart().equals(ENVELOPE)
						...
						if (soapEnvelope != null) {
							try {
								...
								handleException("invalid envelop", axisFault);
							}
						}
					} else {
	...
	/*returns the MappingResourceLoader which uses as an input to generate the final output*/
	...
	/* creates a new mappingResourceLoader if the resources are not available in the cache or when the cache expires*/
	private static MappingResourceLoader createNewMappingResourceLoader(
		...
		mappingResourceMap.put(MAPPING_KEY, new CachedMappingResourceLoader(
